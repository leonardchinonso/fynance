...
- [ ] **Debt accounts (non-mortgage liabilities):** Track borrowed money and student loans as first-class debt accounts, distinct from the existing `Credit` (credit-card) type. Add `Loan` and `StudentLoan` to the `AccountType` enum ([model.rs](../../backend/src/model.rs) `AccountType`, plus its `as_str` / `parse`, and the frontend `ACCOUNT_TYPE_LABELS` / `ACCOUNT_TYPE_COLORS` maps in [colors.ts](../../frontend/src/lib/colors.ts)). Balances are negative liabilities, so they already flow into `total_liabilities`, net worth, and the "By Asset Type" breakdown. Dashboard surfacing: a debt summary on the Portfolio overview showing total outstanding debt plus the aggregate debt repaid over the selected period, summed from transactions categorised as debt repayment (resolved via a stable category key/flag, the same approach as the Category Model work below, not a hardcoded display name). The "Unavailable" tooltip in [portfolio_overview.tsx](../../frontend/src/pages/portfolio/portfolio_overview.tsx) already lists "Mortgage & secured debt"; unsecured / non-mortgage debt should read as its own line. Raised from a UKPF flowchart review of dashboard gaps.
- [ ] **Emergency fund target on the overview:** The `EmergencyFund` account type already exists but is invisible as a concept. Surface it inside the "Income, Spending & Investments" card on the Portfolio overview ([portfolio_overview.tsx](../../frontend/src/pages/portfolio/portfolio_overview.tsx)): the sum of `EmergencyFund`-type account balances, shown against a target expressed as N months of average monthly spending (UKPF guidance is 3-6 months; `avgSpending` is already computed in that card from `cashSummary.spending / monthCount`). Show "X months covered" and a progress bar to the target. The multiplier (and optionally an absolute target amount) is user-configurable, defaulting to 3 months. Raised from a UKPF flowchart review of dashboard gaps.
- [ ] **Financial goals + history milestones:** A new goals primitive. Each goal has a name, target amount, target date, optional linked account/category, and tracked progress. Backend: a `goals` table and CRUD API (`GET/POST/PATCH/DELETE /api/goals`). Dashboard: an active-goals card showing each goal's progress bar (current vs target, time remaining). History: past / completed goals annotate the net-worth history chart as labelled vertical reference lines (Recharts `ReferenceLine` on [styled_line_chart.tsx](../../frontend/src/components/charts/styled_line_chart.tsx), used by [portfolio_history.tsx](../../frontend/src/pages/portfolio/portfolio_history.tsx)) positioned at the goal's date, e.g. "2025 Got married" rendered as a marker on the timeline. Raised from a UKPF flowchart review of dashboard gaps.
...
- [ ] **`get_balance_summary` single-pass rewrite.** It fetches the distinct account ids in range, then loops running MIN/MAX-date and balance lookups per account (4 round-trips each), so the accounts screen cost grows linearly with account count. Replace with one grouped query over holdings computing, per account in a single scan, the first/last snapshot dates and their balances (`ROW_NUMBER() OVER (PARTITION BY account_id ORDER BY as_of)` ascending and descending, or `MIN`/`MAX` plus a join back). The `carried_holdings_sql` machinery from PR #96 is the natural base.
- [ ] **Split `storage/db.rs` (part 1 of the layering fix).** ~7k lines and ~90 methods across every entity. Split into `storage/{transactions,holdings,investments,categories,budgets,documents,meta}.rs` as `impl Db` blocks (one struct's impls can live across files): pure file moves, zero logic change, compiler-verified. Ship as its own logic-free PR immediately after PR #96 merges so it can't conflict with in-flight work.
- [ ] **Move the CGT engine out of the routes layer (part 2).** `run_cgt_engine` and its types (~900 lines of business logic) live in `routes/capital_gains.rs`; move them to `backend/src/cgt/` with the route as a thin adapter. While there, cross-reference the second average-cost pooling implementation in `get_investment_history` against the engine at both sites (they intentionally differ: history skips same-day/30-day matching, and nothing says so today).

### Minor Nits (P3, from the 2026-07 architecture review)

Frontend:

- [ ] Pie chart active-slice center label always formats as GBP: `renderActiveShape` in `interactive_pie.tsx` calls `formatCurrency(value)` with no currency, so USD portfolios show `£` while hovering. Thread the chart's currency through.
- [ ] OS theme switch doesn't repaint charts until interaction: `theme_context.tsx`'s `prefers-color-scheme` listener toggles the DOM class but sets no React state, so `resolvedTheme` consumers (chart palettes) keep stale colors. Track the resolved theme in state inside the listener.
- [ ] Date pickers allow end before start: the custom range pickers in `date_range_selector.tsx` don't constrain the pair, and an inverted range silently renders empty data. Clamp the other bound or swap on selection.
- [ ] Filter order creates duplicate cache entries: data hooks pre-join multi-select arrays (`selectedAccounts.join(",")`) before passing them as query deps, bypassing `stableKey`'s order-independent array sorting, so the same set selected in a different order refetches. Sort before joining, or pass the arrays raw.
- [ ] Investments history filters/sorts/paginates client-side over the full events list while transactions do it server-side. Fine at current event counts; either move it server-side for consistency or document the intentional divergence in the hook.
- [ ] Documents "Links" column resolves reference counts with one `GET /api/documents/:id` per visible row (lazy by design, column hidden by default). Add a batch form (e.g. `?include=refs` on the list endpoint) if the page grows.
- [ ] `shadcn` (the CLI) sits in `package.json` `dependencies`; move to devDependencies or drop it (runs via npx).

Backend:

- [ ] Transaction search doesn't escape backslashes for LIKE (the search filter escapes `%`/`_` only), so a query containing `\` produces malformed escape sequences and wrong matches. Escape `\` first.
- [ ] The `__uncategorized__` filter sentinel works on `GET /api/transactions` but not `/api/transactions/by-category` (it lands in a plain `IN`, and the `category_id IS NOT NULL` predicate excludes those rows regardless). Align the two endpoints' filter contracts.
- [ ] IPv6 `FYNANCE_HOST` cannot parse: `serve.rs` formats `"{host}:{port}"` and `"::1:7433"` is not a valid `SocketAddr`. Bracket IPv6 hosts (`[{host}]:{port}`).
- [ ] Parse progress channels leak on early error paths: `routes/parse.rs` returns before `cleanup_progress()` on preprocessing/account/provider errors, leaving entries alive until the 600s timer. Use a guard type whose `Drop` cleans up.
- [ ] `validate_token` writes `last_used` on every authenticated request, inside the auth middleware, under the global DB lock. Debounce to at most once per minute per token.

Docs:

- [ ] Doc drift: the root `CLAUDE.md` repo map lists `db/migrations`, which doesn't exist (migrations are probe-based in `db.rs` with no `schema_version` table), and `schema.sql` claims profiles are "seeded with a 'default' row on first startup" while `seed_defaults` deliberately does the opposite. Fix both texts.
